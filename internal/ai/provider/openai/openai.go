...
func (p *OpenAIProvider) Detect(ctx context.Context, req *provider.Request) (*provider.Response, error) {
	// 构建提示词
	...
	reqBody := map[string]interface{}{
		...
		"messages": []map[string]string{
			{
				"role":    "system",
				"content": "你是一个敏感信息检测助手。请判断用户提供的文件是否包含敏感信息（如API密钥、密码、私钥、身份证号、银行卡号等）。只返回JSON格式的结果。",
			},
			{
				"role":    "user",
				"content": prompt,
...
func (p *OpenAIProvider) buildPrompt(fileName, content string) string {
...
请按以下JSON格式返回:
{
  ...
  "category": "敏感信息类别(如API_KEY、PASSWORD、PRIVATE_KEY、ID_CARD等)",
  "reason": "判断理由",
  "sensitive_parts": ["列出具体的敏感内容片段"]
}
